...
export default function Register({ onLogin }) {
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await API.post('/auth/register', { name, tenantName, email, password });
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gradient-to-br from-slate-900 via-blue-900 to-slate-800">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-blue-200 text-sm">Start Your Free Account Today</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 backdrop-blur-lg bg-opacity-95">
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold py-3 rounded-lg transition duration-200 flex items-center justify-center gap-2 disabled:opacity-50 mt-6"
            >
              {loading ? (
                <>
          ...
          <p className="text-center text-gray-600 text-xs mt-6">
            By creating an account, you agree to our <br className="hidden sm:inline" />
            <a href="#" className="text-blue-600 hover:text-blue-700">Terms of Service</a> and <a href="#" className="text-blue-600 hover:text-blue-700">Privacy Policy</a>
          </p>
        </div>
      </div>
    </div>
